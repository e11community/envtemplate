...
describe('renderTemplate', () => {
  ...
  it("propagates onMissing: 'error' as a rejection when a var is missing", async () => {
    ...
    await expect(renderTemplate({templatePath, outputPath, onMissing: 'error', env: {}})).rejects.toThrow(/MISSING_KEY/)
  })
  ...
  describe('templatePath fallback', () => {
    ...
    it('throws when all candidates are missing, listing each path in the message', async () => {
      ...
      await expect(renderTemplate({templatePath: [a, b], outputPath, env: {}})).rejects.toThrow(new RegExp(`No template file found.*${a}.*${b}`))
    })
  })
})
